...
- (void)prepareHeaderImage:(PSSpecifier *)specifier
{
	...
	if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) 
 	...
 	if(specifier.properties[@"image"] && specifier.properties[@"background"]) 
 	{
		...
		[headerImageView setFrame:CGRectMake(self.frame.size.width - headerImageView.frame.size.width - 50, 5, headerImageView.frame.size.width, headerImageView.frame.size.height)];
        
        headerImageView.autoresizingMask = (UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth );
